...
import { formatResponseTime } from "@/pages/prompt/playground-tabs/logs/utils/logDetailsHelpers";
...
function getPromptName(promptId: number | undefined, promptNames: PromptName[]) {
	if (!promptId) return "-";
	return promptNames.find((prompt) => prompt.id === promptId)?.name || `Prompt ${promptId}`;
}

function isPromptDeleted(promptId: number | undefined, promptNames: PromptName[]) {
