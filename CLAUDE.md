...
**Email addresses go through `emailSchema` from `@/utils/email` — never a bare `z.email()`.** Zod's default pattern is ASCII-only and rejects both halves of an internationalized address: an IDN domain (`user@müller.de`) and an SMTPUTF8 local part (`jörg@example.com`, RFC 6531). That schema validates against `EMAIL_PATTERN`, which takes zod's `unicodeEmail` and adds back the one thing the old ASCII regex got right — the domain must be two or more non-empty dot-separated labels, so `user@gmailcom` is still a typo and `user@localhost` is deliberately not an account. Crucially it **normalizes before it validates** — trim, NFC, lowercase, NFC again — because a pasted address carries whitespace that the pattern forbids, and because `ö` has two encodings that render identically and compare unequal. `User.email` is `@unique` and looked up with `=`, so an unnormalized write is a duplicate account or a login that cannot find its own row; the second NFC pass is not redundant, since case folding decomposes (`İ` → `i` + U+0307). Addresses that arrive from outside our schemas (identity providers, invitations written by older versions) are compared with `emailsMatch`, not `!==`. `apps/web/src/lib/email.ts` is a deliberate copy of the same two rules — a form that rejects an address never gives the server the chance to accept it. One value is exempt everywhere: `'SYSTEM_USER'` is a sentinel that `ensureSystemUserExists` still looks up by that literal, so lowercasing it strands v1.3.0 upgrades with a second admin.
